...
uniform sampler2D _celShadingSample : register(s0);
uniform float _gm_AlphaRefValue : register(c3);
uniform bool _gm_AlphaTestEnabled : register(c4);
uniform sampler2D _gm_BaseTexture : register(s1);
...
{
{
(gl_Color[0] = ((_v_vColour * gl_texture2D(_gm_BaseTexture, _v_vTexcoord)) * gl_texture2D(_celShadingSample, vec2(_dp, 0.0))));
}
}
;
struct PS_INPUT
